...
- **本番公開URL：** <https://travel-planner-theta-liard.vercel.app/>
  ...
  スマートフォンでの利用を前提にしているので、以下のQRコードを読み込んでご体験いただけますと幸いです。
  - **QRコード**

    <img src="./docs/qr-code.png" alt="QRコード" width="216px">
...
また、「一日のスケジュールが把握しづらい」「スケジュール毎の予算や持ち物を管理すると煩雑になる」といった課題もありました。
...
- 計画中の旅行先をカード形式で一覧表示。旅行期間は設定せずに登録した予定から自動で期間が表示されるようにしました。
...
1. **コールドスタート問題（Renderスリープ制限）の緩和と起動頻度の最適化**

   Renderの無料枠特有の「15分アクセスがないと自動的にサーバーが深く眠ってしまう」という制限に対し、外部監視サービスである **UptimeRobot** を連携させることで解決。5分に1回、自動で内部のエンドポイントをノックさせることで、無料インフラの制約下において**スリープに入る頻度を劇的に低減（高確率でのスリープ回避）**させ、ユーザーがアクセスした際の待機時間を最小限に抑えるインフラの最適化を試みています。
